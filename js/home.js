...
function setRating(e) {
    ...
    for (var i = $currentRating; i >= 1; i--) {
        $('.js-set-rating[data-rating=' + i + '][data-product-id=' + $productId + ']')
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.products, function (index, product) {
                            ...
                            <li class="list-group-item">
                                <div class="clearfix w-100">
                                    <div class="float-left w-50 text-center">
                                        <span 
                                            class="font-italic h4 js-product-price" 
                                            data-price="` + Math.round(100*product.price)/100 + `" 
                                            data-product-id="` + product.id + `"
                                        >
                                            $` + Math.round(100*product.price)/100 + `
                                        </span>
                                    </div>
                                    <div class="float-right w-50 text-center">
                                        <button 
                                            class="btn btn-primary mr-4 js-add-to-cart" 
                                            data-product-id="` + product.id + `"
                                        >
                                            Add to cart
                                        </button>
                ...
                for (var i = 1; i <= 5; i++) {
                    $products += `  <button 
                                        class="btn btn-outline-warning mt-0 js-set-rating" 
                                        data-rating="` + i + `" 
                                        data-product-id="` + product.id + `"
                                    >
                                        ` + i + `
                ...
                $products += `
                                ...
                                <button 
                                    class="btn btn-success js-apply-rating" 
                                    data-product-id="` + product.id + `"
                                >
                                    Apply
                                </button>
